...
> 초안은 12개였다. `[8] Array aggregator`는 실기에서 **중복 차단을 무력화시키는 원인**으로
> 밝혀져 제거했다(아래 [9] 참조).
...
### [8] ~~Array aggregator~~ — **넣지 않는다**

초안에는 [4]와 같은 이유로 여기에도 집계를 두었으나, **실기에서 잘못된 설계로 확인돼 제거했다.**
근거는 아래 [9]의 경고 참조.

### [9] Filter — "중복 아님" (Notion [7] 바로 뒤)

| 왼쪽 | 연산자 | 오른쪽 |
|---|---|---|
| `7. Page ID` | **`Basic operators: Does not exist`** | (비움) |

- 검색이 **찾았으면** 페이지 ID가 있다 → 존재 → **차단** (E-04 정상 스킵)
- **못 찾았으면** 빈 요약 번들이라 ID가 없다 → 없음 → **통과**

👉 **이 필터는 반드시 [10] Gemini보다 앞에 있어야 한다** (설계 원칙 1, NFR-04).

> 🚨 **여기에 Array aggregator를 두면 중복 차단이 통째로 무력화된다.** 실제로 겪은 사고다.
>
> Make의 집계 모듈은 **"Source Module ~ 자기 자신"을 하나의 집계 범위**로 묶는다.
> 그 범위 **안에** 필터를 두면, 필터가 전부 막아도 집계 모듈은 **빈 배열 1건을 내보내며
> 정상 종료**하기 때문에 뒤 모듈이 그대로 실행된다.
> 캔버스에서 `중복아님 ⓧ0` 인데 `HTTP ✓1` 로 찍히는 형태로 드러난다.
>
> 개수로 판정하는 것도 안 된다. **Notion 검색 모듈은 0건일 때도 번들 1개를 내보내므로**
> (`Total number of bundles: 0` 필드를 가진 요약 번들) 집계 배열이 절대 비지 않는다.
> `length(8.array)`는 찾든 못 찾든 항상 `1`이다.
>
> 결론: **집계를 빼고, 필터를 [7] 바로 뒤에 두고, 페이지 ID의 존재 여부로 판정한다.**
> 참조 구현 `prototype/pipeline.py`의 `notion_store.exists()`는 조회 결과를 직접
> 불리언으로 판정하므로 이 함정이 없다.
...
| 항목 | 계산 |
|---|---|
| 정상 저장되는 날 | 약 9~13 ops (Filter는 소모 없음, Aggregator·변수·HTTP는 소모) |
| 정상 스킵되는 날 | 약 4~6 ops |
| 월 30일 운영 | 대략 **270~390 ops** |
| 무료 한도 | 1,000 ops/월 |
...
| **필터가 `ⓧ0`인데 뒤 모듈이 실행됨** | 필터가 **집계 범위 안**(Source Module ~ Aggregator 사이)에 있다. 집계는 입력 0건에도 빈 배열 1건을 내보내며 정상 종료한다 | 필터를 집계 **뒤**로 옮기거나, 그 집계가 불필요하면 제거한다 ([9] 참조) |
| 중복인데 계속 저장됨 | Notion 검색은 **0건에도 번들 1개를 내보낸다.** 개수로 판정하면 항상 `1`이라 절대 안 걸린다 | `Page ID` 의 **존재 여부**로 판정한다 |
