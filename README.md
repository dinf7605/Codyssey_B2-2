...
| 🔧 | Filter `중복 아님` | 조회 결과 | **페이지 ID 존재 여부** 판정 | 통과 or **E-04 종료** |
...
### 집계 모듈이 필요한 곳과, 오히려 해가 되는 곳

**필터 뒤에는 필요하다.** Make는 **필터를 통과한 번들이 0건이면 뒤따르는 모듈이 아예 실행되지
않고 흐름이 끝난다.** 그러면 "오늘은 매칭 기사가 없었다"를 기록하는 것조차 불가능하다.
Array aggregator는 입력이 0건이어도 **빈 배열 1건을 반드시 내보내므로**, 이 단계를 끼워야
뒤에서 `length(array)`로 0건 여부를 판정하고 정상 스킵을 남길 수 있다.

기획 초안의 흐름도에는 이 단계가 없었고, 만들면서 발견해 추가했다.
E-03 주입 테스트가 이 구조의 필요성을 실측으로 증명한다([7장](#e-03--키워드-매칭-0건-정상-스킵)).

**반대로 Notion 검색 뒤에는 넣으면 안 된다.** 처음에는 같은 논리로 여기에도 집계를 두었는데,
그게 **중복 차단을 통째로 무력화**시켰다. 두 가지 이유가 겹쳤다.

1. **Notion 검색 모듈은 0건일 때도 번들 1개를 내보낸다** (`Total number of bundles: 0`
   필드를 가진 요약 번들). 흐름이 끊기지 않으므로 애초에 집계로 방어할 필요가 없었고,
   집계 배열이 절대 비지 않아 **`length()`는 찾든 못 찾든 항상 `1`**이었다.
2. **집계 모듈은 "소스 모듈 ~ 자기 자신"을 하나의 집계 범위로 묶는다.** 그 범위 *안에*
   필터를 두면, 필터가 전부 막아도 집계가 빈 배열 1건을 내보내며 정상 종료하기 때문에
   **뒤 모듈이 그대로 실행된다.** 캔버스에 `중복아님 ⓧ0` 인데 `HTTP ✓1`로 찍혔다.

실제로 같은 기사가 3회 저장됐다. 집계를 제거하고 필터를 Notion 검색 바로 뒤에 두어
**페이지 ID의 존재 여부**로 판정하도록 바꿔 해결했다.

> 참조 구현 `prototype/pipeline.py`는 `notion_store.exists()`가 조회 결과를 직접 불리언으로
> 판정하므로 이 함정이 없었다. **코드로 먼저 확정한 로직이 옳았고, 노코드 툴의 실행 모델을
> 잘못 이해해 생긴 문제였다.**
...
**1회차** — 새 기사를 정상 저장.
**2회차** — 같은 기사로 다시 실행하면 `중복 아님` 필터가 **0**을 반환하고, 그 뒤의
**HTTP `9` · Tools `12` · Notion `15`에 실행 배지가 없다.**
Gemini를 호출하지 않고 종료하며 Notion 행은 1건으로 유지된다.

이것이 "요약 호출 기사당 1회"와 "중복 저장 0건"의 동시 근거다.
**두 번을 연속으로 확인해야 의미가 있다** — 1회차만 보면 중복 차단이 동작하는지 알 수 없고,
2회차만 보면 애초에 저장이 되는지 알 수 없다. 실제로 이 검증을 건너뛰었다가
중복이 3건 쌓인 뒤에야 발견했다.
...
| Notion 검색 뒤 집계 | 있음 | **제거** | 집계 범위 안에 갇힌 필터는 흐름을 끊지 못한다. 중복 3건이 저장된 뒤 발견 |
...
| 담당 | 이름 | 역할 | 실제 작업 내용 | 담당 모듈 |
|---|---|---|---|---|
| A | **김재민** (팀장) | 워크플로우 설계 / 통합 | Make 계정·타임존 설정, 시나리오 통합, 스케줄 트리거, 집계 모듈 배치, 크레딧 모니터링 | 트리거, `5`, `8` |
| B | **고승희** | 데이터 수집 | RSS 피드 조사·선정(`check_feed.py` 실측 비교), 수집 모듈 구성, 주제 필터 정규식 설계 | `2`, 🔧`주제 매칭` |
| C | **장유성** | AI 요약 | Gemini API 키 발급, 모델 선정, 요약 프롬프트 설계, HTTP 모듈, 3줄 후처리 | `9`, `12` |
| D | **김진아** | 저장 / 중복 방지 | Notion DB 스키마 설계, 통합 연결, 중복방지키 규칙 확정, 조회·저장 모듈 | `7`, `15`, 🔧`중복 아님` |
| E | **김종훈** | 품질 / 문서 | 오류 주입 테스트 4종 수행, 에러 처리 정책 문서화, README 총괄, 스크린샷 마스킹 검수 | 전체 검증 |
...
**1. 자동화 워크플로우**
- [x] 매일 설정된 시간에 자동 실행 (09:00 Asia/Seoul) — 연속 실행 기록은 [7장 S1](#s1--자동-실행-기록)
